...
<body>
								...
								<div class="col-sm-12">
									<div class="form-group ">
										<div class="row">
											...
											<div class="col-sm-4">
												<input type="text" class="input" name="page_seo"
	...
	<script>
		$(document).ready(function() {
			...
			$("#pageContent,#pageContentEn,#pageHtml,#pageHtmlEn").kendoEditor({
				tools : [ "bold", "italic",
			...
			global.dataSource = new kendo.data.DataSource({
				transport : {
					read : {
						url : '<c:url value = "/admin/webPages/ajaxWebPagesDlGridList"/>', // 目標ajax網址
						dataType : "jsonp",
						data : function() {
							return {
								pageId : $('#id').val(),
//								searchName : $('#searchName').val()
			...
			$("#grid").kendoGrid({
				...
				// columns 為每個欄位的資訊
				columns : [ {
					// template 可設定在此欄位 要顯示的HTML 資訊 詳情可見 kendoUI 網路文件
					template: 
					 "<button type='button' class='btn btn-success btn-sm' onclick='edit(\"#: id #\")'>編輯</button>&nbsp;",
				...
				dataSource : global.dataSource,// 設定剛才 設定的 dataSource
				resizable: true,
				pageable : true,
				sortable : true
			});
