		...
		<script>
								...
								global.dataSource = new kendo.data.TreeListDataSource(
										{
											transport : {
												read : {
													url : '<c:url value = "/admin/roleManager/ajaxRoleFunGridList"/>?roleType=${roleDto.roleType}&roleNo=${roleDto.roleNo}',
													dataType : "jsonp",
			...
			function cancle(func_id) {
				...
				$
						.ajax({
							type : "POST",
							url : '<c:url value = "/admin/roleManager/ajaxCancelFunc"/>',
			...
			function apply(func_id) {
				...
				$
						.ajax({
							type : "POST",
							url : '<c:url value = "/admin/roleManager/ajaxApplyFunc"/>',
							data : $("#ajaxForm").serialize(),// serializes the form's elements.
							success : function(result) {
								if (checkAjaxResp(result)) {
