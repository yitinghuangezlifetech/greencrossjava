...
<body>
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="card">
					<div class="card-header">${sessionScope.funcName}-帳號功能設定</div>
					<div class="card-body">
						<div class="row ">
							<div class="col-sm-6">
	...
	<script>
							...
							global.dataSource = new kendo.data.TreeListDataSource(
									{
										transport : {
											read : {
												url : '<c:url value = "/admin/subAccountManager/ajaxUserFunGridList"/>?userId=${userDto.id}',
												dataType : "jsonp",
									...
									.kendoTreeList(
											{
												columns : [
														...
														{
															template : "#if(rfid!=''){#"
																	+ "<i class='fas fa-check'> 已套用</i>"
																	+ "#}else{#"
																	+ "<i class='fas fa-times'> 未套用</i>"
																	+ "#}#",
															field : "bIsRoleUse",
															title : "角色套用狀態",
															width : 100
		...
		function cancle(rufId) {
			...
			$
					.ajax({
						type : "POST",
						url : '<c:url value = "/admin/subAccountManager/ajaxCancelFunc"/>',
		...
		function apply(funcId) {
			...
			$
					.ajax({
						type : "POST",
						url : '<c:url value = "/admin/subAccountManager/ajaxApplyFunc"/>',
						data : $("#managerAccountForm").serialize(),// serializes the form's elements.
						success : function(result) {
							if (checkAjaxResp(result)) {
