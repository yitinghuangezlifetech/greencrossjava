...
<body>
	...
	<script>
			...
			global.dataSource = new kendo.data.DataSource(
					{
						transport : {
							read : {
								type : "POST",
								url : '<c:url value = "/admin/workClassShiftsConfigAdmin/ajaxHtGridList"/>',
								dataType : "jsonp",
								data : function() {
									return {
										id:$('#id').val()
			...
			$("#grid").kendoGrid(
					{
						columns : [
								{
									template : "<button type='button' class='btn btn-success btn-sm' onclick='edit(\"#:id#\")'>查閱</button>&nbsp;"
										+"#if(status =='RF'){#"
		...
		function addData(){
			$('#id').val('');
			$('#editForm').attr('action','<c:url value = "/admin/workClassShiftsConfigAdmin/edit"/>');
			...
			warningConfirm(
					...
					function(confirm) {
						if (confirm) {
							...
							$.ajax({
								type : "POST",
								url : '<c:url value = "/admin/workClassShiftsConfigAdmin/ajaxRemoveHt"/>',
