...
<body>
			...
			<form id="qaForm">
				...
				<div class='panel-body'>
					<table class="table table-condensed">
						...
						<tr>
							<td align='center' valign="middle" style="border-top:1px solid #FFF;">
								<label for="">詢問者電話</label>
							</td>
							<td style="border-top:1px solid #FFF;">
								<input type="text" id="questTel" class='input'  value="" style = "width:90%;" readonly> 
							</td>
						</tr>
						<tr>
							<td align='center' valign="middle" style="border-top:1px solid #FFF;">
						...
						<tr>
							...
							<td style="border-top:1px solid #FFF;">
								<input type="text" id="prodName" class='input'  value="" style = "width:90%;" readonly> 
							</td>
						</tr>
						<tr>
							<td align='center' valign="middle" style="border-top:1px solid #FFF;">
								<label for="">產品代號</label>
							</td>
							<td style="border-top:1px solid #FFF;">
								<input type="text" id="prodCode" class='input'  value="" style = "width:90%;" readonly> 
							</td>
						</tr>
						<tr>
							<td align='center' valign="middle" style="border-top:1px solid #FFF;">
	...
	<script>
			...
			global.dataSource = new kendo.data.DataSource(
					{
						transport : {
							read : {
								type : "POST",
								url : '<c:url value = "/admin/productQuestMgr/ajaxGetGridList"/>',
								dataType : "jsonp",
								data : function() {
									return {
										sDate:$('#sDate').val(),
		...
		function chgStatus(status){
			$.ajax({
				type : "POST",
				url : '<c:url value = "/admin/productQuestMgr/ajaxChgQuestStatus"/>',
				data : {
					id : $('#qaId').val(),
					status : status
		...
		function edit(id){
			...
			$.ajax({
				type : "POST",
				url : '<c:url value = "/admin/productQuestMgr/ajaxGetQuest"/>',
				...
				success : function(result) {
					...
					if (checkAjaxResp(result)) {
						...
						$('#questTel').val(result.data.questTel);
						$('#prodName').val(result.data.prodName);
						$('#prodCode').val(result.data.prodCode);
